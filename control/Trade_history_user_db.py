...
class Trade_history_user_mn():
    ...
    def trade_history_user_add(self, point_paid, item_name, item_id, user_id):
        new_history = Trade_history_user(
        id = self.trade_history_user_max_db,
        point_paid = point_paid,
        item_name = item_name,
        item_id = item_id,
        time = time.asctime(time.localtime(time.time() + 60*60*7)),
        user_id = user_id
        )
    ...
    def trade_history_user_scan(self, target_id):
        result_dict = {"UserName": [], "ItemName": [], "Time": [], "Emerald": []}
